...
from scaler.protocol.python.common import ObjectContent, TaskCancelConfirmStatus, TaskResultStatus
...
class SymphonyTaskManager(Looper, TaskManager):
    ...
    async def on_cancel_task(self, task_cancel: TaskCancel):
        ...
        if not task_queued and not task_processing:
            await self._connector_external.send(
                TaskCancelConfirm.new_msg(
                    task_id=task_cancel.task_id, status=TaskCancelConfirmStatus.NotFound, task=None
                )
            )
            return
        ...
        if task_processing and not task_cancel.flags.force:
            await self._connector_external.send(
                TaskCancelConfirm.new_msg(
                    task_id=task_cancel.task_id, status=TaskCancelConfirmStatus.CancelFailed, task=None
                )
            )
            return
        ...
        result = TaskCancelConfirm.new_msg(
            task_id=task_cancel.task_id, status=TaskCancelConfirmStatus.Canceled, task=canceled_task
        )
