...
from scaler.protocol.python.message import InformationCommand, InformationCommandResponse
...
def main():
    ...
    connector.send(InformationCommand.new_msg(command=command_bytes))
    message = connector.receive()
    if not isinstance(message, InformationCommandResponse):
