...
def get_args():
    ...
    parser.add_argument(
        ...
        "-k",
        action="store_true",
        help="store tasks in scheduler when tasks are running, if this is true, when worker disconnected, all the tasks "
        "on this worker will get rerouted to other workers",
    )
    parser.add_argument(
        "--allocate-policy",
        "-ap",
        choices=[p.value for p in AllocatePolicy],
        default=AllocatePolicy.even.value,
