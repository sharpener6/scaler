import logging
from typing import Awaitable, Callable, Optional, Dict, Set, List
...
class VanillaTaskManager(TaskManager, Looper, Reporter):
    def __init__(self, store_tasks: bool, max_number_of_tasks_waiting: int):
        ...
        self._state_functions: Dict[
            TaskState, Callable[[bytes, TaskTransition, TaskStateMachine, Dict], Awaitable[None]]
        ] = {
    ...
    def get_task_paths(self, task_ids: Set[bytes]) -> Dict[str, List[str]]:
        return self._task_state_manager.get_paths(task_ids)
    ...
    async def __routing(self, task_id: bytes, transition: TaskTransition, **kwargs):
        ...
        await self._state_functions[state_machine.state()](task_id, transition, state_machine, kwargs)  # noqa

    async def __state_inactive(
        self, task_id: bytes, transition: TaskTransition, state_machine: TaskStateMachine, kwargs: Dict
    ):
        if transition == TaskTransition.Task:
            task = kwargs["task"]
        elif transition == TaskTransition.SchedulerHasTask:
            return
        elif transition == TaskTransition.TaskCancelConfirmCanceled:
            task = kwargs["task_cancel_confirm"].task
        else:
            raise ValueError(
                f"Task[{task_id.hex()}]: unknown {transition=} in {state_machine.state()}, {state_machine.get_path()}"
            )
    ...
    async def __state_running(
        self, task_id: bytes, transition: TaskTransition, state_machine: TaskStateMachine, kwargs: Dict
    ):
        if transition == TaskTransition.TaskCancelConfirmFailed:
            # if cancel failed (task is ongoing), we should wait here for the result
            return
        elif transition == TaskTransition.HasCapacity:
    ...
    async def __state_canceling(
        ...
        if transition == TaskTransition.TaskCancel:
            client = kwargs["client"]
            task_cancel = kwargs["task_cancel"]
        else:
            raise ValueError(
                f"Task[{task_id.hex()}]: unknown {transition=} in {state_machine.state()}, {state_machine.get_path()}"
            )
    ...
    async def __state_balance_canceling(
        self, task_id: bytes, transition: TaskTransition, state_machine: TaskStateMachine, kwargs: Dict
    ):
        if transition == TaskTransition.BalanceTaskCancel:
    ...
    async def __state_worker_disconnecting(
        self, task_id: bytes, transition: TaskTransition, state_machine: TaskStateMachine, kwargs: Dict
    ):
        if transition == TaskTransition.WorkerDisconnect:
    ...
    async def __state_canceled(
        ...
        if transition == transition.TaskCancel:
            task_cancel_confirm = TaskCancelConfirm.new_msg(
                task_id=task_id, cancel_confirm_type=TaskCancelConfirmType.Canceled, task=None
            )
        elif transition == transition.TaskCancelConfirmCanceled:
            task_cancel_confirm = kwargs["task_cancel_confirm"]
        elif transition == transition.WorkerDisconnect:
            task_cancel_confirm = TaskCancelConfirm.new_msg(
                task_id=task_id, cancel_confirm_type=TaskCancelConfirmType.Canceled, task=None
            )
        else:
    ...
    async def __state_canceled_not_found(
        ...
        if transition == TaskTransition.TaskCancelConfirmNotFound:
            task_cancel_confirm = kwargs["task_cancel_confirm"]
        else:
            raise ValueError(
                f"Task[{task_id.hex()}]: unknown {transition=} in {state_machine.state()}, {state_machine.get_path()}"
    ...
    async def __state_success(
        self, task_id: bytes, transition: TaskTransition, state_machine: TaskStateMachine, kwargs: Dict
    ):
        if transition == TaskTransition.TaskResultSuccess:
    ...
    async def __state_failed(
        self, task_id: bytes, transition: TaskTransition, state_machine: TaskStateMachine, kwargs: Dict
    ):
        if transition == TaskTransition.TaskResultFailed:
            task_result = kwargs["task_result"]
        else:
            raise ValueError(
                f"Task[{task_id.hex()}]: unknown {transition=} in {state_machine.state()}, {state_machine.get_path()}"
    ...
    async def __state_failed_worker_died(
        ...
        if transition == TaskTransition.TaskResultWorkerDied:
            task_result = kwargs["task_result"]
        elif transition == TaskTransition.SchedulerHasNoTask:
            task_result = TaskResult.new_msg(
                task_id=task_id, result_type=TaskResultType.FailedWorkerDied, metadata=b"", results=[]
            )
        else:
            raise ValueError(f"Task[{task_id.hex()}]: unknown {transition=} in {state_machine.state()}")
