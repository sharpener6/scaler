...
class VanillaGraphTaskController(GraphTaskController, Looper, Reporter):
    ...
    async def on_graph_task_cancel(self, client_id: ClientID, task_cancel: TaskCancel):
        print(f"{task_cancel.task_id!r}: received cancel graph task")
        assert self.is_graph_subtask(task_cancel.task_id)
        ...
        print(f"{task_cancel.task_id!r}: start canceling graph {graph_task_id!r}")
        await self.__one_graph_cancel(graph_task_id, task_cancel)
    ...
    async def on_graph_sub_task_result(self, result: TaskResult):
        ...
        await self.__one_graph_done(graph_task_id, result)

    async def on_graph_sub_task_cancel_confirm(self, task_cancel_confirm: TaskCancelConfirm):
        print(f"{task_cancel_confirm.task_id!r}: received cancel confirm")
        graph_task_id = self._task_id_to_graph_task_id[task_cancel_confirm.task_id]
        graph_info = self._graph_task_id_to_graph[graph_task_id]
        if graph_info.status == _GraphState.Canceling:
            return

        await self.__mark_node_canceled(graph_info, task_cancel_confirm)
    ...
    async def __one_graph_done(self, graph_task_id: TaskID, result: TaskResult):
        graph_info = self._graph_task_id_to_graph[graph_task_id]
        ...
        if not self.__is_graph_finished(graph_task_id):
            ...
            await self.__cancel_running_nodes(graph_info)
            await self.__mark_inactive_nodes_done(graph_info, result_status, result_metadata, result_objects)
    ...
    async def __cancel_running_nodes(self, graph_info: _Graph):
        # cancel all running tasks
        await asyncio.gather(
            *[
                self._task_controller.on_task_cancel(graph_info.client, TaskCancel.new_msg(task_id))
                for task_id in graph_info.running_task_ids
            ]
        )

    async def __mark_inactive_nodes_cancelled(self, graph_info: _Graph):
        while graph_info.sorter.is_active():
            ready_task_ids = graph_info.sorter.get_ready()
            for task_id in ready_task_ids:
                await self.__mark_node_canceled(
                    graph_info, TaskCancelConfirm.new_msg(task_id, TaskCancelConfirmType.Canceled)
                )

    async def __mark_node_canceled(self, graph_info: _Graph, task_cancel_confirm: TaskCancelConfirm):
        ...
        if task_cancel_confirm.task_id in graph_info.target_task_ids:
            await self._binder.send(graph_info.client, task_cancel_confirm)

    async def __mark_inactive_nodes_done(
        self,
        graph_info: _Graph,
