...
from scaler.utility.event_loop import create_async_loops
...
class Scheduler:
    ...
    async def on_receive_message(self, source: bytes, message: Message):
        ...
        # =====================================================================================
        # information manager
        if isinstance(message, DebugCommand):
            await self._information_manager.on_debug_command(source, message)
            return
    ...
    async def get_loops(self):
        loops = create_async_loops(
            [
                ...
                (self._information_manager.routine, STATUS_REPORT_INTERVAL_SECONDS),
            ]
        )
