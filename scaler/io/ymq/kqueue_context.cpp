...
TimedQueue::TimedQueue(): _kqfd(-1), _currentId {}
{
    _kqfd = kqueue();
    if (_kqfd == -1) {
...
{
    auto microsecs = convertToKqueueTimer(timestamp);

    // Only set a new timer if this is the earliest event
    if (pq.empty() || timestamp < std::get<0>(pq.top())) {
        struct kevent kev;
        // Use EVFILT_TIMER with NOTE_USECONDS for microsecond precision
        EV_SET(&kev, _currentId, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_ONESHOT, NOTE_USECONDS, microsecs, nullptr);
...
std::vector<TimedQueue::Callback> TimedQueue::dequeue()
{
    ...
    // Set up the next timer if we have more events
    if (!pq.empty()) {
        auto nextTs = std::get<0>(pq.top());
        auto microsecs = convertToKqueueTimer(nextTs);
        auto nextId = std::get<2>(pq.top());

        struct kevent kev;
        EV_SET(&kev, nextId, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_ONESHOT, NOTE_USECONDS, microsecs, nullptr);

        if (kevent(_kqfd, &kev, 1, nullptr, 0, nullptr) == -1) {
            unrecoverableError({
                Error::ErrorCode::CoreBug,
                "Originated from",
                "kevent(2) - setup next timer",
                "Errno is",
                strerror(errno),
            });
        }
...
KqueueContext::KqueueContext()
    : _kqfd(-1)
    , _timingFunctions()
    , _interruptiveFunctions(_kqfd, _isInterruptiveFd)  // Will be updated after kqueue creation
{
    _kqfd = kqueue();
    if (_kqfd == -1) {
        const int myErrno = errno;
        ...
        if (current_event.filter == EVFILT_USER && current_event.ident == _isInterruptiveFd) {
            ...
            if (event) {
                ...
                if (current_event.filter == EVFILT_READ) {
                    ymq_events |= EVFILT_READ;
                }
                if (current_event.filter == EVFILT_WRITE) {
                    ymq_events |= EVFILT_WRITE;
                }
