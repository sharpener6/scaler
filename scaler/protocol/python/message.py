...
class TaskCancelConfirm(Message):
    ...
    @staticmethod
    def new_msg(
        task_id: TaskID, cancel_confirm_type: TaskCancelConfirmType, task: Optional[Task]
    ) -> "TaskCancelConfirm":
        if task is None:
            parameters = dict(taskId=bytes(task_id), cancelConfirmType=cancel_confirm_type.value, noTask=None)
        else:
            parameters = dict(taskId=bytes(task_id), cancelConfirmType=cancel_confirm_type.value, task=task._msg)
